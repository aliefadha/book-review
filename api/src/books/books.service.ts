...
export class BooksService {
    ...
    async createReview(id: string, createReviewDto: CreateReviewDto) {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                this.logger.log(`AI processing attempt ${attempt}/3 for review`);
                ...
                try {
                    ...
                    this.logger.error(`Failed to parse AI response as JSON, using fallback values: ${parseError.message}`, '');
                ...
                this.logger.error(`AI processing attempt ${attempt}/3 failed: ${error.message}`, error.stack || '');

                if (attempt === 3) {
                    // All retries failed - decide whether to save with fallback or reject
                    this.logger.error('All AI processing attempts failed. Rejecting review creation.', '');
